<script lang="ts">
  import { NavLogo } from '$lib/nav/navbar/index.js';
...
<div use:clickOutside={'navdrop-toggle'} on:clickoutside={() => (open = false)}>
  <div
    class="dropdown absolute flex max-w-md w-full items-center justify-between opacity-95 shadow-lg drop-shadow"
    class:open
    on:keydown={(e) => {
...
<style>
  .dropdown {
    ...
    margin: var(--m, 0);
    min-height: var(--min-height, 75vh);
    padding: var(--pd, 10vh 0);
    position: absolute;
    right: -100%;
    top: 0;
