...
<Navbar navClass="px-2 sm:px-4 sticky w-full z-50 top-0 left-0" fluid color="none" let:hidden let:toggle>
  <NavBrand href="/">
    <img src="{logo}" class="mr-3 h-6 sm:h-9" alt="Flowbite Logo"/>
    <span class="self-center whitespace-nowrap text-xl font-semibold dark:prose-invert">{banner}</span>
  </NavBrand>
  <!-- Links -->
  <NavUl {hidden}>  
